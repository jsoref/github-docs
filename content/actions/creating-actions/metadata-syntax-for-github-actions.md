...
If the action is written using a [composite](./creating-a-composite-run-steps-action) then it will not automatically get `INPUT_<VARIABLE_NAME>`. Instead, it is incumbent on the author of the action to manually reimplement this behavior themselves. 
